...
; output file
!ifndef OutFile
    !define OutFile "unishield-agent-${VERSION}-8104.exe"
!endif
...
Section "Unishield 360 Agent (required)" MainSec
    ...
    ; Uses a bundled PowerShell helper (robust against special chars in XML tags)
    ReadEnvStr $0 "WAZUH_MANAGER"
    ${If} $0 != ""
        SetOutPath "$INSTDIR"
        File /oname=patch-ossec-conf.ps1 "patch-ossec-conf.ps1"
        nsExec::ExecToLog 'powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$INSTDIR\patch-ossec-conf.ps1" -Conf "$INSTDIR\ossec.conf"'
...
Section "Unishield 360 Metric Agent (performance metrics)" MetricSec
    ...
    File "metricbeat-oss-8104\metricbeat.exe"
    File "metricbeat-oss-8104\metricbeat.yml"
    File "metricbeat-oss-8104\install-service-metricbeat.ps1"
    File "metricbeat-oss-8104\uninstall-service-metricbeat.ps1"
    File "metricbeat-oss-8104\uninstall-agent.ps1"
    File "metricbeat-oss-8104\register-arp.ps1"
    File "metricbeat-oss-8104\unishield.ico"
    SetOutPath "$INSTDIR\metricbeat-oss\modules.d"
    File "metricbeat-oss-8104\modules.d\windows.yml"
    File "metricbeat-oss-8104\modules.d\system.yml"
    ...
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\Unishield 360 Metric Agent" "DisplayVersion" "8.10.4"
