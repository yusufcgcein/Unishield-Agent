...
function remove_upgrade_files {
    ...
    Remove-Item -Path ".\unishield-agent*.msi" -ErrorAction SilentlyContinue
...
function get_msi_version {
    $msiPath = (Get-Item ".\unishield-agent*.msi").FullName
...
function install {
    ...
    try {
        $msiPath = (Get-Item ".\unishield-agent*.msi").Name
